...
- `{{SIMULATION_STEPS}}` -- the workflow-specific steps a greenfield agent would walk (this workflow's phases/gates).
...
Read only as needed; do not treat as compliance targets unless a binding surface routes the agent to them as an active input:
{{CONTEXT_FILES}}

## Legacy artifacts -- NOT compliance targets
{{LEGACY_ARTIFACTS}}
Treat the above as historical/contextual only. Do NOT issue a finding merely because a legacy artifact fails to match the current guide. Only flag one if (a) the current docs still route agents to it as active surface, (b) the docs fail to distinguish active from historical material, or (c) the workflow still depends on it in a way that would confuse a fresh agent. The test is whether the current docs are coherent and operable now -- not whether old files conform.
...
### 2. Greenfield agent simulation (primary)
...
{{SIMULATION_STEPS}}
At each stop ask: do I find everything I need right here? Do I know exactly when to stop and what gate line to emit? Do I know exactly what to show the operator at each gate? If the workflow only works because you mentally stitch together scattered sections, that is a finding.

### 3. Legacy-encounter simulation (only if legacy artifacts exist)
Simulate a fresh agent hitting a legacy artifact. Do the current docs make its status (active / optional / historical / obsolete) clear? Could it be mistaken for a current template or compliance target? The finding is confusion caused by the current docs, not the existence of old files.
...
1. Findings -- code-review style with file paths + line references; behavioral/process risk, not wording nitpicks. Each finding carries:
   ...
   - **Consequence** -- what happens if the agent follows the document as written. Test in order, first match wins: `irreversible` (destroys work, or leaves the effort's own records unrecoverable or not comparable — ambiguity that changes what an agent does while leaving the inputs auditable is `divergent`, not this) · `divergent` (the agent stops, two agents following the same text do different things, or every agent is reliably sent to the same wrong outcome) · `cosmetic` (neither).
   - **Reachability** -- how the agent gets there. Test in order, first match wins: `past-a-rule` (reaching it requires going against a clear rule the document states, with no competing instruction or worked example sending the agent there) · `normal` (the main path — a step taken on every round counts, even when it usually resolves to nothing) · `off-normal` (a branch reached only when a triggering event occurs: recovery, restart, abandonment, an optional branch).
   ...
   - **Triage** -- your call on how the maintainer should treat it: `accept` (real defect, fix is proportionate) · `log-only` (valid observation, but the fix costs more than the problem) · `operator-decision` (the fix adds process weight -- a new gate, rule, checklist, or proof burden -- not a reviewer's call).
