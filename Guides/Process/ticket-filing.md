...
Purpose: capture process improvement suggestions and file them as GitHub issues on the `skai` repo. Handles both ad-hoc suggestions (the operator describes a problem or idea) and retro handoffs (drafts already written by the retro).
...
- The Phase 1 draft-review gate is the main review/file gate for this workflow. `Next` there means: file the remaining unchecked `- [ ] T<n> Ticket: …` entries now. The operator may instead ask to revise drafts further or leave them for later.
...
- The required `process-tickets.md` draft file cannot be located and the operator has not provided the session/path needed to continue.
...
- Default rule: a `- [ ]` ticket item means pending draft / pending filing. Do not check it without operator approval.
...
- The Phase 1 understanding gate does not require its own workflow artifact or marker; it is an operator approval stop before drafting begins.
- At the Phase 1 draft-review gate, STOP with the ticket items still unchecked.
- If the operator gives advance intent at the Phase 1 draft-review gate, create GitHub issues for each unchecked ticket and update each entry by checking the box (`- [x]`) and appending a `- **Filed** #<number>` sub-bullet.
- If the operator decides to skip an entry during draft review, move it to a `## Skipped` subsection of the file with strikethrough on the title text (the line remains a list item but unchecked); see the example below.
...
The operator reviews all issue content before creation at the draft-review gate. This is the final approval point before filing, but the agent should make a good-faith effort to generalize first.

## Entry points

- **Ad-hoc**: the operator describes a process problem, improvement idea, or feature request. Start at Phase 1.
...
Ask the operator to describe what they have in mind. Use follow-up questions to clarify:
...
If the operator provides a clear description up front, skip the questions and proceed to drafting.
...
If the retro handoff is not specific enough to draft actionable tickets, STOP and ask the operator to clarify before writing the drafts.
...
After a filing run, filed tickets stay in `## Drafts` with the box checked and a `- **Filed** #<n>` sub-bullet appended. The `## Skipped` subsection holds entries the operator chose not to file; the title text gets strikethrough but the checkbox remains unchecked (the entry was neither filed nor "completed" in the workflow's sense).
...
Present the drafts to the operator for review.
...
If it is not available, tell the operator that a GitHub MCP server is required for ticket creation and offer to help set one up (e.g., adding a GitHub MCP server to the project's MCP configuration with a personal access token that has repo issue permissions).
...
Report the created issue numbers and URLs back to the operator.
